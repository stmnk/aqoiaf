...
if st.checkbox(f'Configure Elasticsearch URLs (default value: {LOCAL_URL})'):
    LOCAL_URL = st.text_input(
        'Local Elasticsearch URL', LOCAL_URL, max_chars=500, key='es_local_url')
    BONSAI_URL = st.text_input(
        'Bonsai Elasticsearch URL', max_chars=500, key='es_bonsai_url')
    SEARCHLY_URL = st.text_input(
        'Searchly Elasticsearch URL', max_chars=500, key='es_searchly_url')
    AWS_ES_URL = st.text_input(
        'AWS Elasticsearch URL', max_chars=500, key='es_aws_url')
    CLOUD_ES_URL = st.text_input(
        'Cloud Elasticsearch URL', max_chars=500, key='es_cloud_url')
    ES_URL_STRINGS = [LOCAL_URL, BONSAI_URL,
                      SEARCHLY_URL, AWS_ES_URL, CLOUD_ES_URL]

ACTIVE_URL_LABEL = st.selectbox(
    'Select the active Elasticsearch URL: ', ES_URL_LABELS, key='box_select_url')
...
if not ACTIVE_URL:
    ...
    # localhost URL can be fully shown in the open
    if ACTIVE_URL_LABEL == ES_URL_LABELS[0]:
        st.write(f'Your acive elastcsearch URL is: {ACTIVE_URL}')
    else:                                    # obfuscate sensitive entries in the URL string
        st.write(
            f'Your acive elastcsearch URL is: https://<YOUR_USER>:********@{ACTIVE_URL.split("@")[1]}')

client = Elasticsearch(ACTIVE_URL)
query_body = {"size": 1, "query": {
    "function_score": {"functions": [{"random_score": {
        "seed": int(time.time())}}]}}}
result = client.search(index="document", body=query_body)

st.write(result['hits']['hits'][0]['_source']['text'])
